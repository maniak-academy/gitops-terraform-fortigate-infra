...
resource "fortios_firewall_address" "juiceshop_address" {
  vdomparam               = "FG-traffic"
  name                 = "juiceshop_address"
  associated_interface = "awsgeneve"
  subnet               = "${aws_instance.juiceshop_ec2.private_ip}/32"
  type                 = "subnet"
  visibility           = "enable"
}
...
resource "fortios_firewall_policy" "juiceshop_policy" {
  vdomparam               = "FG-traffic"
  action                      = "accept"
  inspection_mode             = "flow"
  logtraffic                  = "all"
  name                        = "juiceshop_policy"
  schedule                    = "always"
  ...
  dstaddr {
      name = fortios_firewall_address.juiceshop_address.name
  }
